...
modnames = [mname for mname in sys.modules if mname.startswith('pyuutils')]
...
if not os.environ.get("NO_CONTENTS_BUILD"):
    ...
    if os.path.exists(os.path.join(_PROJ_PATH, 'requirements-build.txt')):
        ...
        make_build_cmd = (where.first('make'), 'bin', 'clean', 'build')
...
from pyuutils.config.meta import __TITLE__, __AUTHOR__, __VERSION__
